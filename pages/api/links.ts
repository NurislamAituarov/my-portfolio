...
export const links = [
  ...
  },
  {
    ...
    description: `
    Приложение Регистрации и Аутентификации.
    Регистрация пользователя:
    Пользователь заполняет форму с данными.
    Фронтенд отправляет POST-запрос на /register.
    Бэкенд добавляет нового пользователя в базу данных.
    
    Аутентификация пользователя:
    Пользователь вводит логин и пароль.
    Фронтенд отправляет POST-запрос на /login.
    Бэкенд проверяет данные, создает JWT токен и отправляет его обратно на клиент.
    Фронтенд сохраняет токен и информацию о пользователе в localstorage приложения.
    
    Проверка токена и повторная аутентификация:
    При каждом запросе, требующем авторизацию (например, запрос на защищенные роуты), клиент отправляет токен в заголовке запроса.
    Middleware на бэкенде проверяет токен.
    Если токен истек или недействителен, бэкенд возвращает ошибку аутентификации.
    На фронтенде обработка ошибки аутентификации приводит к перенаправлению на страницу логина для повторной аутентификации.`,
  },
  {
    _id: 'link_6',
    icon: {
